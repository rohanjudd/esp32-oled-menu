...
namespace Menu
{
  ...
  typedef void (*RenderCallback_t)(const Item_t *, byte);
  ...
  public:
    ...
    void render(const RenderCallback_t render, byte maxDisplayedMenuItems) const;
  ...
  void Engine::render(const RenderCallback_t render, byte maxDisplayedMenuItems) const {
    ...
    byte start = 0;
    byte itemCount = 0;
    const byte center = maxDisplayedMenuItems >> 1;
